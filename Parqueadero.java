...
// -----------------------------------------------------------------
    ...
    public String metodo1( )
    {
        ...
        String mensaje = "La cantidad de carros con placa PB: " + cantidadCarrosPB + " - Hay un carro parqueado por 24 o más horas: ";
    ...
    public String metodo2( )
    {
        ...
        int CarroSacados = desocuparParqueadero();
        String mensajero= "La cantidad de carros sacados son" + CarroSacados;
        return mensajero;
    ...
    //2.1
    public float darTiempoPromedio(){
    ...
    public ArrayList<Carro> darCarrosMasDeTresHorasParqueados(){
        ...
        for (Puesto puesto : puestos){
            if (puesto.estaOcupado()){
                Carro carro= puesto.darCarro();
                int horasEnParqueo= carro.darTiempoEnParqueadero((horaActual));
                if(horasEnParqueo>3){
    ...
    public boolean hayCarroMasDeOchoHoras(){
        for (int i=0; i<puestos.length;i++){
            if (estaOcupado(i)){
                int hora=  puestos[i].darCarro().darTiempoEnParqueadero(horaActual);
                if (hora>8){
                    existe = true;
                }
            }
...
//2.2 hecho por la profe
    /*
    for (int i=0; i<puestos.length; i++)

    {
    if (estaOcupado(i)){
       hora=  puestos[i].darCarro().darTiempoEnParqueadero();
       if (hora>8){
       existe =true;
       }
       }
    ...
    public  boolean hayCarrosPlacaIgual(){
        ...
        boolean existe= false;
        for(int i=0; i<puestos.length; i++){
            for (int j=i+1; j<puestos.length-1; j++){
                if (estaOcupado(i)){
                    String placa= puestos[j].darCarro().darPlaca();
                    if (estaOcupado(j)){
                        String placa2= puestos[j].darCarro().darPlaca();
 ...
 * Indica si hay dos carros parqueados con la misma placa.
 * @return true si hay dos carros parqueados con la misma placa, false en caso contrario.

        public boolean hayCarrosConMismaPlaca() {
